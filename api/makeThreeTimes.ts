...
const makeThreeTimes = async (dayType:string,  stNum:string) => {
    let upAndDownData:{upTime:string[]; downTime:string[]} = {upTime: [], downTime: []};
    for(let i = 0; i < 2; i++){
        ...
        if(i === 0){
            upAndDownData.upTime = finalData;
        }else{
            upAndDownData.downTime = finalData;
        }
       
    }
    return upAndDownData
